layui.config({
    ...
    function tablePlay(data) {
        table.render({
            ...
            cols: [//设置表头参数
                [
                    // {type:'checkbox'},
                    {field: 'orderNumber', align: "center", title: '订单号', width: '12%', unresize: true}
                    ...
                    , {field: 'carFactory', align: 'center', title: '整车厂', width: '10%', unresize: true}
                    ...
                    , {field: 'vehicleSystem', align: 'center', title: '整车系统', width: '10%', unresize: true}
                    , {
    ...
    function getCadreList(pageNumber, pageSize) {
        ...
        ajax.request('/mglVehicleCarTotalInfo/list', 'GET', params, function (res) {
    ...
    table.on('tool(test)', function (obj) {
        if (obj.event === 'edit_btn') {//操作—编辑
            // layer.msg('编辑');
            window.location.href = "/changeViews?views=cars/cars-total-info-add&carId=" + obj.data.id;
            ...
            // layer.msg('详情');
            window.location.href = "/changeViews?views=cars/cars-total-info-detail&carId=" + obj.data.id;
        }
    });
    ...
    $("#newTask").click(function () {
        // layer.msg('新建任务');
        window.location.href = "/changeViews?views=cars/cars-total-info-add";
    });

    //删除
    function deleteMission(id) {
        ajax.request("/mglVehicleCarTotalInfo/delete", 'POST', {"id": id}, function (res) {
            layer.msg("删除成功！");
            pageFlag = true;
            getCadreList();
        });
    ...
    $("#exportTask").click(function () {
        ...
        debugger;
        window.location.href = "/mglVehicleCarTotalInfo/exportList?" + requestURL.substring(1);
    });
    ...
    $("#batchExport").click(function () {
        window.location.href = "/changeViews?views=cars/cars-total-info-addMore";
    });
});
